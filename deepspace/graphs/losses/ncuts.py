...
# the ratio of the free space among all gpus
    ...
    def forward(self, seg, padded_seg, weight, sum_weight):
        ...
        cropped_seg = []
        for m in torch.arange((config.deepspace.radius-1)*2+1, dtype=torch.long):
            column = []
            for n in torch.arange((config.deepspace.radius-1)*2+1, dtype=torch.long):
        ...
        return torch.add(-assoc, config.deepspace.K)

    def crop_seg(self, seg):
        cropped_seg = torch.zeros(seg.size()[0], seg.size()[1], seg.size()[2], seg.size()[3], (config.deepspace.radius-1)*2+1, (config.deepspace.radius-1)*2+1)
        padding_size = (config.deepspace.radius, config.deepspace.radius, config.deepspace.radius, config.deepspace.radius)
