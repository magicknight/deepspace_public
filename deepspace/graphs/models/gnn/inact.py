...
form https://github.com/ToruOwO/InteractionNetwork-pytorch
...
class RelationModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.model = nn.Sequential(nn.Linear(input_size, hidden_size), nn.ReLU(), nn.Linear(hidden_size, hidden_size), nn.ReLU(), nn.Linear(hidden_size, hidden_size), nn.ReLU(), nn.Linear(hidden_size, output_size), nn.ReLU())
...
if __name__ == "__main__":
    ...
    data = torch.randn(ts, n_objects, obj_dim)  # shape of data = (ts, n_objects, object_dim)
    # format data
    objects, sender_relations, receiver_relations, relation_info, target = format_data(data, 0)
    # set up network
    model = InteractionNetwork(obj_dim, rel_dim, eff_dim, hidden_obj_dim, hidden_rel_dim)

    parameters = filter(lambda p: p.requires_grad, model.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
    print("Trainable Parameters: %.3fM" % parameters)

    prediction = model(objects, sender_relations, receiver_relations, relation_info)

    print("Shape of out :", prediction.shape)  # [B, in_channels, image_size, image_size]
    print(prediction)
