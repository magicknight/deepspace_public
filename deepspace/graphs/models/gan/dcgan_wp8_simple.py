...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=config.deepspace.image_channels, out_channels=config.deepspace.num_filt_g, kernel_size=10, stride=5, padding=3, bias=False)

        self.conv2 = nn.Conv2d(in_channels=config.deepspace.num_filt_g, out_channels=config.deepspace.num_filt_g * 2, kernel_size=8, stride=4, padding=3, bias=False)
        self.batch_norm1e = nn.BatchNorm2d(config.deepspace.num_filt_g*2)

        self.conv3 = nn.Conv2d(in_channels=config.deepspace.num_filt_g*2, out_channels=config.deepspace.num_filt_g * 4, kernel_size=8, stride=4, padding=3, bias=False)
        ...
        self.conv5 = nn.Conv2d(in_channels=config.deepspace.num_filt_g*8, out_channels=config.deepspace.num_filt_g*16, kernel_size=4, stride=2, padding=1, bias=False)
        self.batch_norm4e = nn.BatchNorm2d(config.deepspace.num_filt_g*16)

        self.conv6 = nn.Conv2d(in_channels=config.deepspace.num_filt_g*16, out_channels=config.deepspace.latent_space_size, kernel_size=4, stride=2, padding=1, bias=False)
        self.batch_norm5e = nn.BatchNorm2d(config.deepspace.latent_space_size)

        self.feature_dim = config.deepspace.image_resolution // 5 // 4 // 4 // 2 // 2 // 2
        self.flatten = nn.Flatten()
        self.linear1 = nn.Linear(self.feature_dim**2 * config.deepspace.latent_space_size, config.deepspace.latent_space_size)
        self.linear2 = nn.Linear(config.deepspace.latent_space_size, self.feature_dim**2 * config.deepspace.latent_space_size)
        self.unflatten = nn.Unflatten(1, (config.deepspace.latent_space_size, self.feature_dim, self.feature_dim))

        self.relu = nn.ReLU(inplace=True)

        self.deconv6 = nn.ConvTranspose2d(in_channels=config.deepspace.latent_space_size, out_channels=config.deepspace.num_filt_g*16, kernel_size=4, stride=2, padding=1, bias=False)
        self.batch_norm6 = nn.BatchNorm2d(config.deepspace.num_filt_g*16)

        self.deconv1 = nn.ConvTranspose2d(in_channels=config.deepspace.num_filt_g*16, out_channels=config.deepspace.num_filt_g * 8, kernel_size=4, stride=2, padding=1, bias=False)
        ...
        self.deconv3 = nn.ConvTranspose2d(in_channels=config.deepspace.num_filt_g * 4, out_channels=config.deepspace.num_filt_g * 2, kernel_size=8, stride=4, padding=2, bias=False)
        self.batch_norm3 = nn.BatchNorm2d(config.deepspace.num_filt_g*2)

        self.deconv4 = nn.ConvTranspose2d(in_channels=config.deepspace.num_filt_g * 2, out_channels=config.deepspace.num_filt_g, kernel_size=8, stride=4, padding=2, bias=False)
        self.batch_norm4 = nn.BatchNorm2d(config.deepspace.num_filt_g)

        self.deconv5 = nn.ConvTranspose2d(in_channels=config.deepspace.num_filt_g, out_channels=config.deepspace.image_channels, kernel_size=10, stride=5, padding=3, output_padding=1, bias=False)
