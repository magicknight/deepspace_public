...
from deepspace.graphs.losses.bce import BinaryCrossEntropy
...
class DCGANAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # define dataloader
        self.dataloader = CelebADataLoader(config)

        # define loss
        self.loss = BinaryCrossEntropy()

        # define optimizers for both generator and discriminator
        self.optimG = torch.optim.Adam(self.netG.parameters(), lr=config.learning_rate, betas=(config.beta1, config.beta2))
        self.optimD = torch.optim.Adam(self.netD.parameters(), lr=config.learning_rate, betas=(config.beta1, config.beta2))
        ...
        self.fixed_noise = Variable(torch.randn(config.batch_size, config.g_input_size, 1, 1))
        self.real_label = 1
        self.fake_label = 0

        # set cuda flag
        self.is_cuda = torch.cuda.is_available()
        if self.is_cuda and not config.cuda:
        ...
        # Summary Writer
        self.summary_writer = SummaryWriter(log_dir=config.summary_dir, comment='DCGAN')

    def load_checkpoint(self, file_name):
        filename = config.checkpoint_dir + file_name
        try:
            ...
            self.logger.info("Checkpoint loaded successfully from '{}' at (epoch {}) at (iteration {})\n"
                             .format(config.checkpoint_dir, checkpoint['epoch'], checkpoint['iteration']))
        except OSError as e:
            self.logger.info("No checkpoint exists from '{}'. Skipping...".format(config.checkpoint_dir))
    ...
    def save_checkpoint(self, file_name="checkpoint.pth.tar", is_best=0):
        ...
        # Save the state
        torch.save(state, config.checkpoint_dir + file_name)
        # If it is the best copy it to another file 'model_best.pth.tar'
        if is_best:
            shutil.copyfile(config.checkpoint_dir + file_name,
                            config.checkpoint_dir + 'model_best.pth.tar')
    ...
    def train(self):
        for epoch in range(self.current_epoch, config.max_epoch):
    ...
    def train_one_epoch(self):
        ...
        for curr_it, x in enumerate(tqdm_batch):
            ...
            fake_noise = torch.randn(x.size(0), config.g_input_size, 1, 1)
    ...
    def finalize(self):
        ...
        self.save_checkpoint()
        self.summary_writer.export_scalars_to_json("{}all_scalars.json".format(config.summary_dir))
