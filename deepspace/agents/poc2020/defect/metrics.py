        ...
        with torch.no_grad():
            for test_images, labels, paths in tqdm_batch:
                ...
                for test_image, label in zip(test_images, labels):
                    ...
                    diff_image = torch.where(diff_image > config.deepspace.image_threshold[1], zero_array, diff_image)
                    diff_image = torch.where(diff_image > config.deepspace.image_threshold[0], one_array, zero_array)
                    diff_image = diff_image.squeeze()
                    defect_area = torch.sum(diff_image, (1, 2, 0))
                    defect_label = (defect_area > config.deepspace.area_threshold).type(torch.float)
