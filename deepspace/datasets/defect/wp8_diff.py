...
class PNGImages:
    ...
    def __getitem__(self, index):
        ...
        """
        # get image path
        metric_index = (index + 10) % len(self.images_path)
        image_path = self.images_path[index]
        metric_image_path = self.images_path[metric_index]
        # read in images
        image = imageio.imread(image_path)
        metric_image = imageio.imread(metric_image_path)
        if self.transform is not None:
            image = self.transform(image)
            metric_image = self.transform(metric_image)
        return image, metric_image
