...
def save_img(img_array, root, index=None, progress_bar=True):
    ...
    for i in batches:
        ...
        # cv2.imwrite(str(Path(root) / (str(i) + ".jpg")), result)
...
def real_to_anime():
    ...
    result_array = []
    # get data loader
    data_loader = get_dataloader(Path(config.deepspace.original_video_img), batch=config.deepspace.anime_batch)
    # 开始转换
    ...
    # 保存图片
    save_img(result_array, config.deepspace.anime_video_img)
    return
...
#     result_array = []
#     # get data loader
#     data_loader = get_dataloader(Path(config.deepspace.anime_video_img), batch=config.deepspace.hs_batch)
#     # 开始转换
...
#                 continue
#         # 保存图片
#         save_img(result.permute(0, 2, 3, 1).cpu().numpy(), config.deepspace.high_video_img, index=index)
#         # result_array.append(result.permute(0, 2, 3, 1).cpu().numpy())
#     # list to numpy array
#     # result_array = np.concatenate(result_array, axis=0)
#     # 保存图片
